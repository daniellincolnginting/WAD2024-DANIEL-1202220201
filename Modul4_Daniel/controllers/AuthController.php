...
class AuthController
{
    ...
    public function login()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (password_verify($password, $data['password'])) {
                ...
                $_SESSION ["mesage"] = "login berhasil";
    
                // Cek apakan Rememmber Me ter ceklis dan Buat Cookie
                if (isset($input["remember"])) {
                    setcookie("nim", "password", time() + (86400 * 30), "/"); // 30 hari
                } 
                else {
                    setcookie("nim", "password", time() - 3600, "/"); // hapus cookie
                }
    ...
    private function generateNIM($id_pendaftaran){
        $conn = $this->conn;
        $query = "GET * FROM pendaftaran WHERE id_pendaftaran = '$id_pendaftaran'";
